...
import { ChevronDown, ChevronUp, Inbox, DollarSign, Calendar, User, LayoutGrid, List, ChevronRight, GripVertical } from 'lucide-react';
import { format } from 'date-fns';

// ─── Column definitions ───────────────────────────────────────────────────────
// Each column defines how a job maps to it (match), and what status/action to
// apply when a job is dropped into it (applyDrop).
...
const COLUMNS = [
  {
    ...
    match: (job) => !job.scheduled_date && job.status !== 'Cancelled' && job.status !== 'No-Show',
    applyDrop: () => ({ status: 'Scheduled', scheduled_date: null }),
  },
  {
    id: 'scheduled',
    label: 'Scheduled',
  ...
  },
  {
    ...
    applyDrop: () => ({ status: 'In Progress', started_at: new Date().toISOString() }),
  },
  {
    id: 'completed',
    label: 'Done',
    ...
    applyDrop: () => ({ status: 'Completed', completed_at: new Date().toISOString() }),
  },
  {
    id: 'invoiced',
    label: 'Invoiced',
    ...
    applyDrop: null, // invoice must be created through the job detail page
  },
  {
    id: 'paid',
    label: 'Paid',
    ...
    applyDrop: null, // payment handled through invoice
  },
  {
    id: 'cancelled',
    label: 'Cancelled',
    ...
    applyDrop: () => ({ status: 'Cancelled', cancelled_at: new Date().toISOString() }),
  },
];
...
function JobCard({ job, customer, invoice, col, index }) {
  return (
      ...
      {(provided, snapshot) => (
        <div
          ...
          className={`bg-white rounded-lg border p-3 group transition-shadow ${
            snapshot.isDragging
              ? 'shadow-lg border-slate-300 rotate-1'
        ...
        >
          <div className="flex items-start gap-1.5">
            ...
            <div
              ...
              className="mt-0.5 flex-shrink-0 text-slate-300 hover:text-slate-400 cursor-grab active:cursor-grabbing"
            >
            ...
            <div className="flex-1 min-w-0">
              <Link
                ...
                className="flex items-start justify-between gap-1 group/link"
              >
                <p className="text-sm font-semibold text-slate-800 leading-snug line-clamp-2 flex-1 group-hover/link:text-blue-600 transition-colors">
                  {job.title}
                </p>
                <ChevronRight className="w-3.5 h-3.5 text-slate-300 group-hover/link:text-blue-400 flex-shrink-0 mt-0.5" />
              </Link>
              ...
              {(job.estimated_cost || invoice?.amount) && (
                <div className="flex items-center gap-1 mt-1">
...
function KanbanColumn({ col, jobs, customerMap, invoiceMap }) {
  const total = jobs.reduce((sum, j) => sum + (invoiceMap[j.id]?.amount || j.estimated_cost || 0), 0);
  const isReadOnly = col.applyDrop === null;

  return (
    <div className="flex flex-col w-[220px] flex-shrink-0">
      {/* Header */}
      <div className={`flex items-center justify-between px-3 py-2 rounded-t-lg ${col.headerBg} border ${col.lightBorder} border-b-0`}>
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${col.dot}`} />
          <span className={`text-xs font-semibold ${col.textColor}`}>{col.label}</span>
          {isReadOnly && <span className="text-xs text-slate-400" title="Drop not supported — manage via job detail">🔒</span>}
        </div>
      ...
      <Droppable droppableId={col.id} isDropDisabled={isReadOnly}>
        {(provided, snapshot) => (
          <div
            ...
            className={`flex-1 rounded-b-lg border ${col.lightBorder} p-2 space-y-2 min-h-[140px] transition-colors ${
              snapshot.isDraggingOver && !isReadOnly
                ? `${col.lightBg} border-dashed`
                : 'bg-white'
            }`}
          >
            {jobs.length === 0 && !snapshot.isDraggingOver && (
              <div className="text-xs text-slate-300 text-center pt-6 select-none">
                {isReadOnly ? 'Managed via invoices' : 'Drop here'}
              </div>
            )}
            {jobs.map((job, index) => (
              <JobCard
      ...
      {/* Total */}
      {total > 0 && (
        <div className={`text-xs text-center py-1.5 ${col.lightBg} ${col.textColor} font-medium border ${col.lightBorder} border-t-0 rounded-b-lg`}>
    ...
    <div className="bg-white border rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {inbound.length > 0 && (
            <span className="text-xs bg-red-100 text-red-600 font-semibold px-2 py-0.5 rounded-full">{inbound.length}</span>
          )}
        </div>
          ...
          <div className="divide-y max-h-72 overflow-y-auto">
            {activeItems.map(msg => (
              <div key={msg.id} className="px-4 py-3 flex items-start gap-3 hover:bg-slate-50">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-slate-800 truncate">{customers[msg.customer_id]?.name || 'Unknown'}</p>
...
function ListView({ jobs, customerMap, invoiceMap }) {
  if (jobs.length === 0) return <div className="bg-white border rounded-xl p-8 text-center text-slate-400 text-sm">No jobs found.</div>;
  return (
      ...
      {jobs.map(job => {
        ...
        return (
          ...
          >
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${col.lightBg} ${col.textColor} flex-shrink-0`}>{col.label}</span>
...
export default function JobPipelineView({ businessId }) {
  ...
  const onDragEnd = async ({ source, destination, draggableId }) => {
    ...
    const destCol = COLUMN_MAP[destination.droppableId];
    if (!destCol || !destCol.applyDrop) return;

    const updates = destCol.applyDrop();

    // Optimistic update
    setJobs(prev => prev.map(j => j.id === draggableId ? { ...j, ...updates } : j));

    try {
      await base44.entities.Job.update(draggableId, updates);
    } catch (e) {
  ...
  const totalRevenue = Object.values(invoiceMap).filter(i => i.status === 'Paid').reduce((s, i) => s + (i.amount || 0), 0);
  const outstanding = Object.values(invoiceMap).filter(i => i.status !== 'Paid' && i.status !== 'Cancelled').reduce((s, i) => s + (i.amount || 0), 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Outstanding', value: `$${Number(outstanding).toLocaleString()}`, color: 'text-orange-600' },
          { label: 'Collected', value: `$${Number(totalRevenue).toLocaleString()}`, color: 'text-green-600' },
        ].map(stat => (
